...
[TestClass]
public class GetRelatedUsersAsyncShould
{
    [TestMethod]
    public async Task ReturnTheUsersThatTheRepositoryReturns()
    {
        ...
        // Act
        var result = await userService.GetRelatedUsersAsync(userA.Id, CancellationToken.None).ToArrayAsync();

        // Assert
        Assert.AreEqual(2, result.Length);
        Assert.AreEqual(userB.Id, result[0].Id);
        Assert.AreEqual(userC.Id, result[1].Id);
    }
}
